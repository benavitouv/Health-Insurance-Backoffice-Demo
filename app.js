...
form.addEventListener('submit', async (event) => {
  ...
  if (!file) {
    setStatus('error', 'אנא צרפו מסמך בקשה לביטוח לפני השליחה.');
    return;
  }

  submitBtn.disabled = true;
  form.classList.add('is-loading');
  setStatus('info', 'מעלה את המסמך ושולח את בקשת הביטוח...');
  ...
  try {
    ...
    setStatus(
      'success',
      'בקשת הביטוח נשלחה בהצלחה! צוות הביטוח יחזור אליכם בקרוב.'
    );
    form.reset();
    setFile(null);
    showSuccessModal();
